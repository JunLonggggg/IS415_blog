...
```{r cache=TRUE, eval=FALSE}
bw.fixed <- bw.gwr(formula = resale_price ~ remaininglease_years + floor_area_sqm + prx_to_hawker + prx_to_trainStn  + prx_to_supermarket, data=resale_tbl.sp, approach="CV", kernel="gaussian", adaptive=FALSE, longlat=FALSE)
```
...
```{r cache=TRUE, eval=FALSE}
gwr.fixed <- gwr.basic(formula = resale_price ~ remaininglease_years + floor_area_sqm + prx_to_hawker + prx_to_trainStn  + prx_to_supermarket, data=resale_tbl.sp, bw=bw.fixed, kernel = 'gaussian', longlat = FALSE)
```
